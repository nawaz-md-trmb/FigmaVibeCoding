      ...
      <AiUxSpecCard>
        <div className="relative max-w-xl">
          ...
          <ModusWcTypography
            hierarchy="p"
            size="sm"
            customClass="rounded-md border border-dashed border-[var(--modus-wc-color-base-200)] bg-[var(--modus-wc-color-base-100)] p-3 !m-0 leading-relaxed text-[var(--modus-wc-color-base-content)]"
          >
...
function resolveInlineActionParagraph(host: HTMLElement | null): HTMLElement | null {
  if (!host) return null;
...
function AiUxInlineActionPreviewInteractive() {
  ...
  const typographyHostRef = useRef<HTMLElement | null>(null);
  ...
  const bindTypographyHost = useCallback((node: HTMLElement | null) => {
    typographyHostRef.current = node;
    paragraphRef.current = resolveInlineActionParagraph(node);
  }, []);
  ...
  useLayoutEffect(() => {
    paragraphRef.current = resolveInlineActionParagraph(typographyHostRef.current);
    const p = paragraphRef.current;
    if (p) {
  ...
  return (
    <AiUxSpecCard>
      <div ref={rootRef} className="relative max-w-xl">
        ...
        <ModusWcTypography
          ref={bindTypographyHost}
          hierarchy="p"
          size="sm"
          customClass="cursor-text select-text rounded-md border border-dashed border-[var(--modus-wc-color-base-200)] bg-[var(--modus-wc-color-base-100)] p-3 !m-0 leading-relaxed text-[var(--modus-wc-color-base-content)]"
          label="The contractor shall submit weekly progress reports including schedule variance, open risks, and photos of critical path work. This sentence could be tightened before sharing with the owner."
        />
